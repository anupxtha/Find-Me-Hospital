...
public interface AppointmentRepo extends CrudRepository<Appointment, Integer> {
    ...
    @Query(value = "select a.month, ifnull(b.total,0) as total from (select m.month, count(u.requested_appointment_date) as total from (select 1 as month union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7 union all select 8 union all select 9 union all select 10 union all select 11 union all select 12) m left join `find-me-hospital`.appointment u on month(u.requested_appointment_date) = m.month where u.patient_id = :pId group by m.month) b Right Join (select 1 as month union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7 union all select 8 union all select 9 union all select 10 union all select 11 union all select 12) a on b.month = a.month group by a.month", nativeQuery = true)
    public List<Map<Integer, Integer>> appCountWithDateforPatient(@Param("pId") int pId);
    
    @Query(value = "select a.month, ifnull(b.total,0) as total from (select m.month, count(u.requested_appointment_date) as total from (select 1 as month union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7 union all select 8 union all select 9 union all select 10 union all select 11 union all select 12) m left join `find-me-hospital`.appointment u on month(u.requested_appointment_date) = m.month join `find-me-hospital`.hos_ser_doc hsd on u.hospital_service_doctor_id = hsd.id join `find-me-hospital`.hospital h on hsd.hospital_id = h.id where h.id = :hId group by m.month) b Right Join (select 1 as month union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7 union all select 8 union all select 9 union all select 10 union all select 11 union all select 12) a on b.month = a.month group by a.month", nativeQuery = true)
    public List<Map<Integer, Integer>> appCountWithDateforHospital(@Param("hId") int hId);
}
